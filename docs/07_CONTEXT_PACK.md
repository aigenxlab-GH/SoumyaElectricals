...
> **Usage:** Paste this file's contents at the start of every new Claude Code session.
> It is also saved as `CLAUDE.md` at the monorepo root and auto-loaded by Claude Code.
...
- Employee logs overtime entry (payout = hours × current system rate at time of creation)
...
- `GET|POST|POST /timecards` · `PATCH|DELETE /timecards/:id` · `POST /timecards/bulk`
- `GET|POST|PATCH|DELETE /overtime` · `PATCH|DELETE /overtime/:id`
- `GET|POST|PATCH|DELETE /leaves` · `GET /leaves/balance` · `POST /leaves/bulk`
- `GET|POST /approvals/timecards` · `POST /approvals/timecards/:id` · same for leaves
- `GET|POST|PATCH /users` · `GET /users/:id` · `GET /users/reportable`
- `GET|PUT /config`
- `GET /calendar`
- `GET /dashboard`
...
├── docs/               ← project documentation (00–08); read this file at session start
├── backend/
│   ├── src/
