 ...
 *
 * ⚠️  PREFER backend/scripts/wipe-products.sql via Supabase SQL Editor.
 *     That file does the same thing in one transaction, including the
 *     sequence reset (which this script can't do via the JS client).
 *
 * Run: node backend/scripts/wipe-products.mjs
 * Reads credentials from backend/.env (never hardcode the service-role key).
 */
